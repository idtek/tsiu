...
void GameEngine::DoInit()
{
	...
	//g_poEngine->GetRenderMod()->SetRendererWin()
	...
	for(int i = 0; i < kRowCount; ++i)
	{
		for(int j = 0; j < kColumnCount; ++j)
		{
			...
			newBlock->InitBlock(nextX, nextY, blockWidth, blockHeight, "TileBlank.bmp", "Tile1.bmp", GSBlock::EControlFlag_FreeSize);
			char objName[64];
			sprintf(objName, "Image%d_%d", i, j);
			GetSceneMod()->AddObject(objName, newBlock);

			nextX = blockWidth * (j + 1);
		}
		nextY = blockHeight * (i + 1);
		nextX = 0;
	}
}
